...
public class AidConverter {
    ...
    final private static String aidTable = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz-_";
    ...
    public static long fn2aidu(String fn){
        ...
        // Utils.LOG("AidConverter","fn2aidu     if( !isNumeric(v1String) || v1String.length() != 10 ) return 0;");
        ...
        try {
            ...
            if(urlList.size()>1&&urlList.size()<6){
                for(int i=0;i<urlList.size();i++){
                    if(urlList.get(i).indexOf(".htm")!=-1||urlList.get(i).indexOf(".html")!=-1){
                        fileWhere=i;
                    }
                }
                if(fileWhere>1){
