    ...
    fun onBind(data: PartialPost, selected: Boolean) {
        binding.apply {
            ...
            if (adapterPosition % 2 == 0) {
                articleListItemMain.setBackgroundColor(getColor(context, R.attr.darkGreyTwo))
            } else {
                articleListItemMain.setBackgroundColor(getColor(context, R.attr.black))
            }

            if (selected) {
                articleListItemTextViewTitle.setTextColor(getColor(context, R.attr.tangerine))
            } else {
                if (data.read) {
                    articleListItemTextViewTitle.setTextColor(getColor(context, R.attr.blueGrey))
                } else {
                    articleListItemTextViewTitle.setTextColor(getColor(context, R.attr.paleGrey))
                }
            }
        }
    }
