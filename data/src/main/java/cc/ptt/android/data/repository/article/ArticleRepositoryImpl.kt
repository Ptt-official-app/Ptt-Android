...
class ArticleRepositoryImpl constructor(
    ...
    override fun getArticleDetail(
        boardId: String,
        articleId: String,
    ): Flow<ArticleDetail> = articleRemoteDataSource.getArticleDetail(boardId, articleId).flowOn(Dispatchers.IO)

    override fun getArticleComments(
        boardId: String,
        articleId: String,
        desc: Boolean,
    ): Flow<ArticleCommentsList> = articleRemoteDataSource.getArticleComments(boardId, articleId, desc).flowOn(Dispatchers.IO)
    ...
    override fun createArticleComment(
        ...
        type: Int,
        content: String,
    ): Flow<ArticleComment> = articleRemoteDataSource.createArticleComment(bid, aid, type, content).flowOn(Dispatchers.IO)

    override fun postArticleRank(
        rank: Int,
        boardId: String,
        articleId: String,
    ): Flow<ArticleRank> = articleRemoteDataSource.postArticleRank(rank, boardId, articleId).flowOn(Dispatchers.IO)
}
